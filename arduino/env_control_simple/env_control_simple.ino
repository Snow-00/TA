...
// setpoint
#define SP_LOW_TEMP 23
#define SP_HIGH_HUMID 85
#define SP_HIGH_TEMP 24
#define SP_LOW_HUMID 80
...
void loop() {
  ...
  if (avgHumid <= SP_LOW_HUMID) {
    driveAct("pump", 0);
  }
  else if (avgHumid >= SP_HIGH_HUMID) {
    driveAct("pump", 1);
  }

  if (avgTemp <= SP_LOW_TEMP) {
    driveAct("peltier", 1);
  }
  else if (avgTemp >= SP_HIGH_TEMP) {
    driveAct("peltier", 0);
  }
}
